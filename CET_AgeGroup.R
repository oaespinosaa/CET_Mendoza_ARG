...
afiliados_grupo <- read_excel(file,sheet = 'Afil_grupoedad') %>%
  select(Año,grupo_edad,afiliados_totalgrupo) %>% left_join(gs_pc %>% select(Año,prop),by = 'Año') %>%
  mutate(afiliados_grupoedad = afiliados_totalgrupo*prop)
aux_afilgrupo <- afiliados_grupo %>% group_by(grupo_edad) %>%
  summarise(val2016 = afiliados_grupoedad[Año == 2016],.groups = 'drop')
afiliados_grupo <- afiliados_grupo %>%
  bind_rows(expand.grid(Año = 2014:2015,grupo_edad = c('0-14','15-64','65+')) %>%
              left_join(aux_afilgrupo %>% rename(afiliados_grupoedad = val2016),by = 'grupo_edad')) %>% 
  arrange(Año) %>% select(Año,grupo_edad,afiliados_grupoedad)
...
qaly <- readRDS('Data/QALY_mendoza_year_age.rds') %>%
  mutate(grupo_edad = case_when(age_group5 %in% c('0','1-9','10-14')~'0-14',
                                age_group5 %in% paste0(seq(15,60,5),'-',seq(19,64,5))~'15-64',
                                age_group5 %in% c(paste0(seq(65,75,5),'-',seq(69,79,5)),'80+')~'65+')) %>%
  group_by(anio,grupo_edad) %>% summarise(QALY = sum(QALY),.groups = 'drop')
yll <- readRDS('Data/YLL_mendoza_year_age.rds') %>% 
  mutate(grupo_edad = case_when(age_group5 %in% c('0','1-9','10-14')~'0-14',
                                age_group5 %in% paste0(seq(15,60,5),'-',seq(19,64,5))~'15-64',
                                age_group5 %in% c(paste0(seq(65,75,5),'-',seq(69,79,5)),'80+')~'65+')) %>%
  group_by(anio,grupo_edad) %>% summarise(YLL = sum(YLL),.groups = 'drop')

# Consolidated table
df_uce_tot <- qaly %>% full_join(yll,by = c('anio','grupo_edad')) %>%
  left_join(afiliados_grupo,by = c('anio' = 'Año','grupo_edad')) %>% 
...
# Compute CET by age group =========================================
aux <- NULL
for (age in c('0-14','15-64','65+')){
  df_uce <- df_uce_tot %>% filter(grupo_edad == age)
...
for (i in seq_len(nrow(reg_posibles))) {
  ...
  aux <- bind_rows(aux,reg_posibles %>% mutate(grupo_edad = age))
}
